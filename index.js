...
class Seasons {
  ...
  next() {
    // ✨ implement
    let current = this.seasons[this.currentSeason]
...
class Car {
  ...
  drive(distance) {
    // ✨ implement
    let max = this.tank * this.mpg
    if(distance <= max) { this.odometer = this.odometer + distance
  ...
  refuel(gallons) {
    ...
    return Car} else { this.tank += gallons}
    return Car
  }
}
